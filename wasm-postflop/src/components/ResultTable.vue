  ...
  setup(props) {
    ...
    const exportSummary = () => {
      ...
      if (props.tableMode !== "chance") {
          ...
          const header = ["Hand", "OpponentRange", "Weight", "Combos", "CurrentBoard", "Player", "SelectedActions"];
          if (!props.results.isEmpty) {
              header.push("Equity", "EV", "EQR");
          }
          ...
          if (numActions.value > 0) {
              ...
              for (let i = actions.length - 1; i >= 0; --i) {
                  ...
                  header.push(`${action.name}${amount} %`);
                  header.push(`${action.name}${amount} EV`);
              }
          }
          data.push(header.join(","));

          const currentBoardStr = props.currentBoard ? props.currentBoard.map(cardStr).join(" ") : "-";
          const opponentIndex = props.displayPlayer === "oop" ? 1 : 0;

          // 遍历 props.spots，生成 SelectedActions 数据
          const selectedActions: string[] = [];
          for (const spot of props.spots) {
            console.log("spot", spot);
            if (spot.type === "player") { // 判断是否是 SpotPlayer 类型
                const spotPlayer = spot as SpotPlayer; // 明确类型断言
                for (const action of spotPlayer.actions) {
                    if (action.isSelected) {
                      const combinedAction = action.amount === "0" ? action.name : `${action.name} ${action.amount}`;
                      selectedActions.push(combinedAction); // 添加组合后的动作
                    }
                }
            }
          }
          const selectedActionsStr = selectedActions.join(";");

          const opponentRange = props.cards && props.cards[opponentIndex]
              ? props.cards[opponentIndex].reduce<string[]>((acc, cardCode, index) => {
                  const weight = props.results?.weights[opponentIndex][index] || 0;
                  const normalizer = props.results?.normalizer[opponentIndex][index] || 0;
                  if (weight !== 0 && normalizer !== 0) {
                      const card1 = cardCode & 0xff;
                      const card2 = cardCode >>> 8;
                      acc.push(cardStr(card2) + cardStr(card1));
                  }
                  return acc;
              }, [])
              : [];

          const maxLength = Math.max(resultsSorted.value.length, opponentRange.length);

          for (let i = 0; i < maxLength; i++) {
              const row = resultsSorted.value[i] || [];
              const opponentEntry = opponentRange[i] || "";

              const hand = row.length > 0 ? `${cardStr(row[0] >>> 8)}${cardStr(row[0] & 0xff)}` : "";
              ...
              const rowData = [
                  ...
                  i === 0 ? currentBoardStr : "",
                  i === 0 ? props.displayPlayer : "",
                  i === 0 ? selectedActionsStr : "",
                  ...row.slice(props.results.isEmpty ? INDEX_STRATEGY_BASE : INDEX_EQUITY),
              ];
          ...
          const header = [columns.value[0].label, "OpponentRange", "Combos", "Equity", "EV", "EQR", "CurrentBoard", "Player", "SelectedActions"];
          if (numActions.value > 0) {
              const actions = (props.selectedSpot as SpotPlayer).actions;
              for (let i = actions.length - 1; i >= 0; --i) {
                  const action = actions[i];
          ...
          for (const spot of props.spots) {
              if (spot.type === "player") { // 判断是否是 SpotPlayer 类型
                  ...
                  for (const action of spotPlayer.actions) {
                      if (action.isSelected) {
                          selectedActions.push(action.name); // 添加选中的动作名称
                      }
                  }
              }
          }
          ...
          const opponentRange = props.cards && props.cards[opponentIndex]
              ? props.cards[opponentIndex].map((cardCode) => {
                  const card1 = cardCode & 0xff;
                  const card2 = cardCode >>> 8;
                  return cardStr(card2) + cardStr(card1);
              })
              : [];

          const maxRows = Math.max(resultsSorted.value.length, opponentRange.length);
